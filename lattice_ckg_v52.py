...
class LatticeCKGv5:
    ...
    def __init__(self, cfg):
        ...
        self.H = {k: [] for k in [
            ...
            'h_trace','h_diverg','box_h','svt_tensor','svt_scalar',
            'bose_kl','anis_l1','anis_l2'
        ]}
    ...
    def _setup(self):
        ...
        # Равновесная нормировка — Бозе-Эйнштейн (Планк)
        T_ph = c['T_phys']
        # Болцман (классич.): f = exp(-ω/T)
        # Бозе-Эйнштейн:     f = 1/(exp(ω/T)-1)  [квантовый предел]
        # Используем Бозе при T_phys < om_max/3, иначе Больцман
        if T_ph < self.om.max()/3:
            f = 1.0 / (np.exp(self.om / T_ph) - 1 + 1e-6)  # Планк
            self.ic_type = 'Bose-Einstein'
        else:
            f = np.exp(-self.om / T_ph)  # Больцман
            self.ic_type = 'Boltzmann'
    ...
    def _box_h(self, h):
        """
        □h_αβ residual — волновое уравнение.
        □h = (∂²_t - ∇²)h. Мы вычисляем только ∇²h (∂_t через разностную схему).
        Возвращает std(∇²h) — должен убывать к 0 для свободных гравитонов.
        """
        dx = self.dx
        # Лапласиан h по пространственным осям (оси 2,3,4 в h[mu,nu,x,y,z])
        lap_h = np.zeros_like(h)
        for ax in [2, 3, 4]:
            lap_h += (np.roll(h,-1,ax) - 2*h + np.roll(h,1,ax)) / dx**2
        # Усредняем по TT-компонентам (поперечно-бесследным)
        # TT: 1,2 компоненты (поперечные к z)
        box_TT = float(np.std(lap_h[1,1]) + np.std(lap_h[1,2]) + np.std(lap_h[2,2]))
        return box_TT

    def _svt_decompose(self, h):
        """
        SVT-разложение (скалярный-векторный-тензорный).
        В k-пространстве: тензорные моды (TT) = гравитоны, должны выживать.
        Скалярные/векторные = должны затухать (нет физических источников).
        Возвращает: (scalar_power, vector_power, tensor_power)
        """
        Nx = self.Nx
        # Фурье-образ h (берём h_11 как пример)
        h11 = h[1,1]   # (Nx,Nx,Nx)
        H11 = np.fft.rfftn(h11)
        # Полная спектральная мощность
        P_total = float(np.mean(np.abs(H11)**2))
        # TT-проекция в k-пространстве: убираем продольные моды
        # Простая оценка: сравниваем h_11 с h_22 (должны быть равны для TT)
        h22 = h[2,2]
        H22 = np.fft.rfftn(h22)
        P_tensor = float(np.mean(np.abs(H11 - H22)**2))  # должно быть мало
        P_scalar = float(np.mean(np.abs(H11 + H22)**2))  # след
        return P_scalar, P_tensor, P_total

    def _bose_T(self):
        """
        Проверка Бозе-статистики: T(ω) должно следовать спектру Планка.
        n_B(ω) = 1/(exp(ω/T_phys) - 1)
        Сравниваем текущее T(ω) (усред. по x,θ,φ) с n_B(ω).
        """
        T_om = np.mean(self.T, axis=(0,1,2,4,5))   # (N_om,)
        T_phys = self.c['T_phys']
        n_B = 1.0 / (np.exp(self.om / T_phys) - 1 + 1e-10)
        # Нормируем
        T_norm = T_om / (T_om.sum() + 1e-20)
        n_norm = n_B / (n_B.sum() + 1e-20)
        # Расстояние от планковского спектра
        kl_div = float(np.sum(T_norm * np.log(T_norm / (n_norm + 1e-20) + 1e-20)))
        return kl_div, T_om, n_B
    ...
    def _diag(self, verbose=True):
        ...
        if verbose:
            ...
            print(f"           | h^μ_μ={ht:.2e} | ∂h={hdiv:.2e}"
                  f" | □h={box_h:.2e} | Bose_KL={kl:.3f}")
            if P_tot > 1e-20:
                print(f"           | SVT: scalar={P_s/P_tot:.3f} tensor={P_t/P_tot:.3f}")
